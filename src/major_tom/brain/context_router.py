...
class IntelligentContextRouter:
    ...
    def get_working_summary(self, task_id: str) -> str:
        """Get task-local summary for VLM prompt conditioning."""
    ...
    def _save_cache(self) -> None:
        try:
            ...
            if len(self.cache) > self._CACHE_MAX_SIZE:
                ...
                for k in keys[: len(self.cache) - self._CACHE_MAX_SIZE]:
                    del self.cache[k]
    ...
    def shutdown(self) -> None:
        """Shut down the thread pool executor, waiting for in-flight tasks."""
        if self._current_future and not self._current_future.done():
