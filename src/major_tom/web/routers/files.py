...
def _get_category(path: Path) -> str:
    ...
    path_str = str(path)
    if "experiments/configs" in path_str or "experiments\\configs" in path_str:
        return "configs"
    elif "Memory" in path_str:
        return "memory"
    elif "Record" in path_str:
        return "records"
...
def get_current_config():
    ...
    try:
        config_path = ALLOWED_PATHS["config_json"]
